...
{
  $query = getDataBase()->prepare("SELECT * FROM user WHERE last_name =  ?");
...
{
  $query = getDataBase()->query("SELECT * FROM user");
...
{
  $query = getDataBase()->prepare('INSERT INTO user(first_name, last_name, password, mail, phone, status) VALUES(:first_name, :last_name, :password, :mail, :phone, :status)');
...
{
  $selectLast = getDataBase()->query('SELECT id_user FROM user ORDER BY id_user DESC LIMIT 0, 1');
